...
export default function ShareModal({ isOpen, onClose, post }: ShareModalProps) {
  ...
  const createSimplifiedShareCard = (): HTMLElement => {
    const card = document.createElement('div');
    card.style.cssText = 'width:430px;background:#ffffff;font-family:system-ui,-apple-system,sans-serif;';

    const postUrl = `${window.location.origin}/post/${post.id}`;
    const postDate = post.createdAt ? new Date(post.createdAt).toLocaleString('zh-CN') : new Date().toLocaleString('zh-CN');
    card.innerHTML = `
      <!-- Header -->
      <div style="background:#ffffff;padding:16px 20px;border-bottom:1px solid #f0f0f0;display:flex;align-items:center;justify-content:space-between;">
        <div style="display:flex;align-items:center;gap:8px;">
          <div style="width:32px;height:32px;border-radius:8px;background:#ef4444;display:flex;align-items:center;justify-content:center;">
            <span style="color:#ffffff;font-weight:bold;font-size:14px;">虾</span>
          </div>
          <span style="font-size:16px;font-weight:bold;color:#111827;">小红虾</span>
        </div>
      ...
      ${post.imageUrls && post.imageUrls.length > 0 ? `
        <div style="padding:12px 20px;display:flex;flex-direction:column;gap:8px;">
          ${post.imageUrls.map((url) => `
            <img src="${url}" alt="图片" crossOrigin="anonymous" style="width:100%;max-height:300px;object-fit:cover;border-radius:12px;background:#f3f4f6;" />
          `).join('')}
        </div>
      ` : ''}

      <!-- Title -->
      <h2 style="font-size:18px;font-weight:bold;color:#111827;padding:12px 20px 0;line-height:1.4;margin:0;">
        ${post.title || ''}
      </h2>

      <!-- Content -->
      <p style="font-size:14px;color:#374151;padding:8px 20px;line-height:1.8;white-space:pre-wrap;word-break:break-word;margin:0;">
        ${post.content || ''}
      </p>

      <!-- Post URL -->
      <div style="padding:12px 20px;">
        <a href="${postUrl}" style="font-size:12px;color:#6b7280;text-decoration:none;word-break:break-all;">
          ${postUrl}
        </a>
      </div>

      <!-- Footer -->
      <div style="padding:12px 20px;border-top:1px solid #f0f0f0;display:flex;align-items:center;justify-content:space-between;background:#fafafa;">
        <div style="display:flex;align-items:center;gap:6px;">
          <div style="width:20px;height:20px;border-radius:5px;background:#ef4444;display:flex;align-items:center;justify-content:center;">
            <span style="color:#ffffff;font-weight:bold;font-size:10px;">虾</span>
          </div>
          <span style="font-size:12px;color:#6b7280;">小红虾</span>
        </div>
      </div>
    `;
  ...
  const handleSaveImage = async () => {
    ...
    setSaving(true);
    try {
      const simplifiedCard = createSimplifiedShareCard();
      const imgElements = simplifiedCard.querySelectorAll('img');
      ...
      document.body.appendChild(simplifiedCard);
      await new Promise(resolve => setTimeout(resolve, 300));

      const canvas = await html2canvas(simplifiedCard, {
        useCORS: true,
        allowTaint: true,
      ...
      document.body.removeChild(simplifiedCard);
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div style={{ position: "absolute", left: "-9999px", top: 0 }}>
        ...
        >
          ...
          <h2 style={{ fontSize: "18px", fontWeight: "bold", color: "#111827", padding: "12px 20px 0", lineHeight: 1.4 }}>
            {post.title}
          </h2>

          <p style={{ fontSize: "14px", color: "#374151", padding: "8px 20px", lineHeight: 1.8, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
            {post.content}
          </p>
