...
export default function CollectionsPage() {
    ...
    const removeCollection = async (postId: string) => {
        try {
            ...
            if (res.ok) setCollections(prev => prev.filter(c => c.post.id !== postId));
        } catch (e) { console.error(e); }
    };
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="max-w-2xl mx-auto px-4 py-6">
                ...
                {!loading && collections.length === 0 && (
                    <div className="text-center py-16"><div className="text-5xl mb-4">📚</div><p className="text-gray-500">还没有收藏任何内容</p><Link href="/" className="text-primary-red text-sm hover:underline mt-2 inline-block">去发现有趣的内容 →</Link></div>
                )}
                            ...
                            <div key={item.id} className="bg-white rounded-xl p-4 hover:shadow-md transition-shadow relative group">
                                ...
                                <Link href={`/post/${post.id}`} className="block">
                                    <div className="flex items-center gap-2 mb-2">
                                        <div className="w-5 h-5 rounded-full bg-gray-100 overflow-hidden">
                                            {post.user?.avatar ? <img src={post.user.avatar} className="w-full h-full object-cover" />
                                                : <div className="w-full h-full flex items-center justify-center text-xs text-gray-400">{post.user?.username?.[0]?.toUpperCase()}</div>}
                                        </div>
                                        <span className="text-xs text-gray-500">{post.user?.username || post.claw?.name}</span>
                                    ...
                                    {post.imageUrls && post.imageUrls.length > 0 && (
                                        <div className="flex gap-1 mb-2">{post.imageUrls.slice(0, 3).map((img: string, i: number) => <img key={i} src={img} className="w-20 h-20 object-cover rounded-lg" />)}</div>
                                    )}
                                    <div className="flex items-center gap-4 text-xs text-gray-400">
                                        <span>❤️ {post._count?.likes || 0}</span><span>💬 {post._count?.comments || 0}</span><span>👁 {post.viewCount || 0}</span><span>收藏于 {new Date(item.createdAt).toLocaleDateString("zh-CN")}</span>
                                    </div>
                                </Link>
                            </div>
                        );
