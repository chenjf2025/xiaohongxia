        ...
        <div className="min-h-screen bg-gray-50">
                ...
                <div className="max-w-2xl mx-auto px-4 py-4">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-3">
                            ...
                            {unread > 0 && <button onClick={() => markRead()} className="text-sm text-gray-500 hover:text-gray-900 hover:underline">全部已读</button>}
                        </div>
                    </div>
                </div>
            </div>
            <div className="max-w-2xl mx-auto px-4 py-6">
                ...
                {!loading && displayed.length === 0 && (
                    <div className="text-center py-16"><div className="text-5xl mb-4">🔔</div><p className="text-gray-500">{filter === "unread" ? "暂无未读通知" : "暂无通知"}</p></div>
                )}
                <div className="space-y-2">
                    {displayed.map((n: any) => (
                        <div key={n.id} className={`p-4 bg-white rounded-xl transition-colors ${!n.isRead ? "border-l-4 border-gray-900" : ""}`} onClick={() => !n.isRead && markRead(n.id)}>
                            <div className="flex items-start gap-3">
                                ...
                                {!n.isRead && <div className="w-2 h-2 bg-gray-900 rounded-full flex-shrink-0 mt-2"></div>}
                            </div>
                            {n.targetId && (
                                <Link href={n.targetType === "post" ? `/post/${n.targetId}` : `/profile/${n.targetId}`}
                                    className="text-xs text-gray-500 hover:text-gray-900 hover:underline mt-2 inline-block ml-8" onClick={e => e.stopPropagation()}>
                                    查看详情 →
                                </Link>
                            )}
                        </div>
