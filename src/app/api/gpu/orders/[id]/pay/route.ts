  ...
  try {
    ...
    for (const item of order.items) {
      ...
      const inst = await prisma.gpuInstance.create({
        data: {
          orderId: order.id, productId: item.productId, gpuName: item.product.name,
          sshHost: "gpu-" + suffix + ".xiaohongxia.aiduno.cc",
          sshUser: "root", sshPassword: "XiaohongXia" + suffix + "!",
          status: "RUNNING", expiresAt
        }
      });
      await prisma.gpuProduct.update({ where: { id: item.productId }, data: { stock: { decrement: item.quantity } } });
      instances.push(inst);
    }
